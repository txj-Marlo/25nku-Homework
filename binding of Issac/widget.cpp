    ...
    , ui(new Ui::GameWidget)
{
    ui->setupUi(this);
    widget =this;
    setWindowTitle("The Binding of Issac");
    setWindowIcon(QIcon(":/icons/img/windowicon.png"));
    setFixedSize(900, 600);

    mGameView.setParent(this);
    mGameView.setSceneRect(QRect(0,0,900,600));
...
{
    mStartScene.setSceneRect(QRect(0,0,900,600));
    //设置背景图
    mStartScene.addPixmap(QPixmap(":/icons/img/OIP-C.jpg"));
...
void GameWidget::Init_Game()
{
    ...
    //设置图片
    mGameScene.setSceneRect(QRect(0,0,900,600));
    mGameScene.addPixmap(QPixmap(":/backgrouds/img/Gameroom.png"));
    ...
    connect(&mPlayer,&Player::LevelUp,[this](){
        ...
        optBtn1->move(150,120);
        optBtn2->move(350,120);
...
void GameWidget::GameWin()
{
    ...
    //配置结束场景
    gameClock->hide();
    mWinScene.setSceneRect(QRect(0,0,900,600));
    ...
    winText->setScale(0.8);
    mWinScene.addItem(winText);
    continueBtn->hide();
    restartBtn->show();
    restartBtn->move(300,200);
    backBtn->show();
    backBtn->move(300,260);
    exitBtn->show();
    exitBtn->move(300,320);
...
void GameWidget::GameOver()
{
    ...
    //配置结束场景
    gameClock->hide();
    mOverScene.setSceneRect(QRect(0,0,900,600));
    mOverScene.addPixmap(QPixmap(":/backgrouds/img/gameover.png"));
    loseText = new GameObject();
    loseText->setPixmap(QPixmap(":/ui/img/ui/loseText.png"));
    loseText->setScale(0.8);
...
void GameWidget::playerMove()
{
    ...
    if(mPlayer.pos().y()>600-mPlayer.scale()*mPlayer.pixmap().height())
        mPlayer.setY(600-mPlayer.scale()*mPlayer.pixmap().height());
}
...
void GameWidget::enemySummon(int _HP, int _Attack, qreal _Speed, int _exp, int _No)
{
    ...
    do
    {
        QPointF _pos(QRandomGenerator::global()->bounded(-50,950),QRandomGenerator::global()->bounded(10,650));
        tpos = _pos + QPointF(Enemy->pixmap().width()*Enemy->scale()/2,Enemy->pixmap().height()*Enemy->scale()/2);

    }while(abs((tpos - mPlayer.getCenterPos()).x()) < 400 && abs((tpos - mPlayer.getCenterPos()).y()) < 250);
    ...
    connect(Enemy,&EnemyBase::isDead,[this,Enemy](){
        ...
        auto Bloodstain = new bloodstain(Enemy->getCenterPos());
        mGameScene.addItem(Bloodstain);
    });
...
void GameWidget::setProperties()
{
    ...
    //细节属性栏配置
    mProperties.setPixmap(QPixmap(":/ui/img/ui/Attributes.png"));
    mProperties.setPos(50,260);
...
{
    if(mPlayer.isInvincible) return;//金身
    int enemyNum = mEnemyList.size();
...
void GameWidget::bullet_enemyCollision()
{
    ...
    int enemyNum = mEnemyList.size();
    for(int i=0;i< bulletNum;i++)//遍子弹
    {
        for(int j=0;j<enemyNum;j++)//遍历敌人
        {
...
void GameWidget::releaseOutBullet()
{
    for(int i=0;i<mBulletList.size();i++)
    {
        if(mBulletList[i]->getCenterPos().x()>950 || mBulletList[i]->getCenterPos().x()<-50
            || mBulletList[i]->getCenterPos().y()>650 || mBulletList[i]->getCenterPos().y()<-50)
        {
            mGameScene.removeItem(mBulletList[i]);
            if (mBulletList[i] != nullptr) mBulletList[i]->deleteLater();
            mBulletList.removeOne(mBulletList[i]);
...
void GameWidget::releaseOutEnemyBullet()
{
    ...
    {
        if(mEnemyBulletList[i]->getCenterPos().x()>950 || mEnemyBulletList[i]->getCenterPos().x()<-50
            || mEnemyBulletList[i]->getCenterPos().y()>650 || mEnemyBulletList[i]->getCenterPos().y()<-50)
        {
            mGameScene.removeItem(mEnemyBulletList[i]);
            if (mEnemyBulletList[i] != nullptr) mEnemyBulletList[i]->deleteLater();
            mEnemyBulletList.removeOne(mEnemyBulletList[i]);
